...
Public Class studentforms
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        lstdetails.Columns.Add("Guardian Name", 120)
        lstdetails.Columns.Add("Contact", 100)
        Call con()
        Call fill_account()
    End Sub
    ...
    Private Sub GenerateNewStudentID()
        Dim dbPath As String = "C:\Users\PC\source\repos\project_ADBMS\project_ADBMS\bin\Debug\adbms.mdb"
        Dim connectionString As String = $"Provider=Microsoft.Jet.OLEDB.4.0;Data Source={dbPath};"
        Dim yearPart As String = DateTime.Now.Year.ToString().Substring(2)
        ...
        Try
            Using conn As New OleDbConnection(connectionString)
                ...
                Dim query As String = "SELECT STD_ID FROM Students ORDER BY STD_ID DESC"
                Using cmd As New OleDbCommand(query, conn)
                    Using reader As OleDbDataReader = cmd.ExecuteReader()
                        While reader.Read()
                            Dim lastID As String = reader("STD_ID").ToString().Trim().ToUpper()

                            If Regex.IsMatch(lastID, $"^GC-{yearPart}\d{{4}}$") Then
                                Dim numericPart As String = lastID.Substring(5)
                                If Integer.TryParse(numericPart, nextNum) Then
                                    nextNum += 1
                                End If
                                Exit While
                            End If
            ...
            Dim newID As String = $"GC-{yearPart}{nextNum.ToString("D4")}"
            Call clear()

        Catch ex As Exception
            MessageBox.Show("Error generating Student ID: " & ex.Message)
        End Try
    End Sub
    ...
    Private Sub btnadd_Click(sender As Object, e As EventArgs) Handles btnadd.Click
        ...
        ElseIf txtparent.Text = "" Then
            MsgBox("Enter Guardian Name", vbInformation, "Missing")
            txtparent.Focus()
            Exit Sub
